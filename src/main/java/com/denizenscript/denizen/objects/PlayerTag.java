...
public class PlayerTag implements ObjectTag, Adjustable, EntityFormObject, FlaggableObject {
    ...
    public static void notePlayer(OfflinePlayer player) {
        ...
        if (!playerNames.containsKey(CoreUtilities.toLowerCase(player.getName()))) {
            playerNames.put(CoreUtilities.toLowerCase(player.getName()), player.getUniqueId().getUUID());
        }
    }
    ...
    public static PlayerTag valueOfInternal(String string, TagContext context, boolean defaultAnnounce) {
        ...
        if (string.length() <= 16 && playerNames.containsKey(string)) {
            OfflinePlayer player = new OfflinePlayer(playerNames.get(string));
    ...
    public PlayerTag(OfflinePlayer player) {
        this(player.getUniqueId().getUUID());
    }
...
//    }
    ...
    public LocationTag getLocation() {
        if (isOnline()) {
            return new LocationTag(getPlayerEntity().position());
        }
        else {
    ...
    public int getRemainingAir() {
        if (isOnline()) {
            return getPlayerEntity().getAirSupply();
        }
        else {
    ...
    public int getMaximumAir() {
        if (isOnline()) {
            return getPlayerEntity().getMaxAirSupply();
        }
        else {
            return 300;
        }
    ...
    public int getFoodLevel() {
        if (isOnline()) {
            return getPlayerEntity().getFoodData().getFoodLevel();
        }
        else {
    ...
    public LocationTag getEyeLocation() {
        if (isOnline()) {
            return new LocationTag(getPlayerEntity().getEyePosition());
        }
        else {
            return null;
        }
...
//todo find correct menutype for workbench
    public CraftingMenu getBukkitWorkbench() {
        if (isOnline())
        {
            if (getPlayerEntity().containerMenu instanceof CraftingMenu)
            {

            }
                return (CraftingMenu) getPlayerEntity().containerMenu;
        }
        return null;
    }
    ...
    public WorldTag getWorldTag() {
        if (isOnline()) {
            return new WorldTag(getPlayerEntity().getLevel().dimensionType().toString());
        }
        else {
    ...
    public ItemTag getHeldItem() {
        return new ItemTag(getPlayerEntity().getMainHandItem());
    }

    public ItemTag getOffhandItem() {
        return new ItemTag(getPlayerEntity().getOffhandItem());
    }


    //todo check which stats have keys
//    public void decrementStatistic(Stat<?> statistic, int amount) {
//        ServerPlayer serverPlayer = getPlayerEntity();
//        serverPlayer.getStats().getValue();
//        getOfflinePlayer().decrementStatistic(statistic, amount);
//        server.getPlayerList().getPlayers().resetStat
//    }
//
...
////        if (statistic.getType() == Stats.ANIMALS_BRED .Type.ENTITY) {
...
//    {
//        setBedSpawnLocation(new BlockPos(location.toPosition()));
//    }
//    public void setBedSpawnLocation(BlockPos location) {
//        if (isOnline()) {
//            ServerPlayer serverPlayer = getPlayerEntity();
//            getPlayerEntity().setRespawnPosition(serverPlayer.level.dimension(), location, serverPlayer.getYRot(), false, true);
//        }
//        else {
...
//        if (isOnline()) {
//            ServerPlayer serverPlayer = getPlayerEntity();
//            getPlayerEntity().setRespawnPosition(serverPlayer.level.dimension(), serverPlayer.level.sp, location.getYRot(), false, true);
//        }
//        else {
//            getNBTEditor().setBedSpawnLocation(location);
//        }
//    }
    ...
    public void setLocation(Location location) {
        if (isOnline()) {
            ServerPlayer serverPlayer = getPlayerEntity();
            serverPlayer.teleportTo(location.serverLevel, location.getX(), location.getY(), location.getZ(), serverPlayer.getYRot(), serverPlayer.getXRot());
        }
        else {
...
//    }
    ...
    public void setRemainingAir(int air) {
        if (isOnline()) {
            getPlayerEntity().setAirSupply(air) ;
        }
        else {
...
//    }
    ...
    public void setFoodLevel(int foodLevel) {
        if (isOnline()) {
            getPlayerEntity().getFoodData().setFoodLevel(foodLevel);
        }
        else {
    ...
    public float getExp() {
        if (isOnline()) {
            return getPlayerEntity().experienceProgress;
        }
        else {
    ...
    public void setExp(float xp) {
        if (isOnline()) {
            getPlayerEntity().experienceProgress = xp;
        }
        else {
    ...
    public int getTotalExperience() {
        if (isOnline()) {
            return getPlayerEntity().totalExperience;
        }
        else {
    ...
    public void setTotalExperience(int xp) {
        if (isOnline()) {
            getPlayerEntity().totalExperience = xp;
        }
        else {
    ...
    public int getLevel() {
        if (isOnline()) {
            return getPlayerEntity().experienceLevel;
        }
        else {
    ...
    public void setLevel(int level) {
        if (isOnline()) {
            getPlayerEntity().experienceLevel = level;
        }
        else {
...
//        ForgeChunkManager chunkManager = ((ServerChunkProvider) getPlayerEntity().getLevel().getChunkProvider()).chunkManager;
//        chunkManager
//        ServerLevel serverLevel = ServerLifecycleHooks.getCurrentServer().getLevel().getChunk();
...
//        });
        ...
        registerOnlineOnlyTag(ObjectTag.class, "target", (attribute, object) -> {
            ...
            ServerPlayer serverPlayer = ServerLifecycleHooks.getCurrentServer().getPlayerList().getPlayer(object.uuid);
            Vec3 StartPosition = serverPlayer.getEyePosition();
            Vec3 TargetVector = serverPlayer.getLookAngle().multiply(new Vec3(range, range, range));
            AABB searchBox = serverPlayer.getBoundingBox().expandTowards(TargetVector.scale(range)).inflate(1.0D, 1.0D, 1.0D);
            EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult(
                    serverPlayer.level,
                    serverPlayer,
                    StartPosition,
                    StartPosition.add(TargetVector),
                    searchBox,
                    (e) -> {
                        if (e.getUUID().equals(object.getUUID())) {
                            return false;
                        }
                        if (matcher != null) {
            ...
            return new EntityTag(entityHitResult.getEntity()).getDenizenObject();
        });
...
//            if (object.isOnline())
//            {
//                ServerPlayer serverPlayer = object.getPlayerEntity();
//                BlockPos respawnposition = serverPlayer.getRespawnPosition();
//                if (respawnposition != null)
//                {
//                    return new Location(ServerLifecycleHooks.getCurrentServer().getLevel(serverPlayer.getRespawnDimension()), );
//                }
//                else
//                {
//                    return null;
...
//            }
//
//            Location bedSpawn = NMSHandler.playerHelper.getBedSpawnLocation(object.getOfflinePlayer());
...
//                Location calculatedBedSpawn = object.getPlayerEntity().getRespawnPosition();
...
//        });
        ...
        registerOfflineTag(ElementTag.class, "exhaustion", (attribute, object) -> {
            if (object.isOnline()) {
                return new ElementTag(object.getPlayerEntity().getFoodData().getExhaustionLevel());
            }
            else {
...
//        });
        ...
        tagProcessor.registerTag(ElementTag.class, "is_banned", (attribute, object) -> {
            ...
            return new ElementTag(ServerLifecycleHooks.getCurrentServer().getPlayerList().getBans().isBanned(object.getGameProfile()));
        ...
        tagProcessor.registerTag(ElementTag.class, "is_op", (attribute, object) -> {
            return new ElementTag(ServerLifecycleHooks.getCurrentServer().getPlayerList().isOp(object.getOfflinePlayer().gameProfile));
        });
        ...
        tagProcessor.registerTag(ElementTag.class, "whitelisted", (attribute, object) -> {
            return new ElementTag(ServerLifecycleHooks.getCurrentServer().getPlayerList().isWhiteListed(object.getOfflinePlayer().gameProfile));
            }, "is_whitelisted");
...
//        });
        ...
        registerOnlineOnlyTag(ElementTag.class, "list_name", (attribute, object) -> {
            if (object.isOnline())
            {
                return new ElementTag(object.getPlayerEntity().getScoreboardName().toString(), true);
            }
            else
            {
        ...
        registerOnlineOnlyTag(ElementTag.class, "display_name", (attribute, object) -> {
            if (object.isOnline())
            {
                return new ElementTag(object.getPlayerEntity().getDisplayName().toString(), true);
            }
            else
            {
                return new ElementTag(object.getName(), true);
        ...
        tagProcessor.registerTag(ElementTag.class, "name", (attribute, object) -> {
            if (attribute.startsWith("list", 2) && object.isOnline()) {
                ...
                attribute.fulfill(1);
                return new ElementTag(object.getPlayerEntity().getScoreboardName(), true);
            }
            if (attribute.startsWith("display", 2) && object.isOnline()) {
                BukkitImplDeprecations.playerNameTags.warn(attribute.context);
                attribute.fulfill(1);
                return new ElementTag(object.getPlayerEntity().getDisplayName().getString(), true);
            }
            return new ElementTag(object.getName(), true);
        });
...
//        });
        ...
        registerOnlineOnlyTag(InventoryTag.class, "open_inventory", (attribute, object) -> {
            return InventoryTag.mirrorBukkitInventory(object.getPlayerEntity().containerMenu);
        });
        ...
        // -->
        //todo only found method recipebook.contains
//        registerOnlineOnlyTag(ListTag.class, "discovered_recipes", (attribute, object) -> {
//            ListTag result = new ListTag();
//            for (NamespacedKey recipe : object.getPlayerEntity().getRecipeBook()) {
...
//        });
        ...
        // -->
        //todo client side but can get all offers
//        registerOnlineOnlyTag(ElementTag.class, "selected_trade_index", (attribute, object) -> {
//            if (object.getPlayerEntity().containerMenu instanceof MerchantMenu) {
//                return new ElementTag(((MerchantMenu) object.getPlayerEntity().containerMenu).getOffers());
//            }
//            return null;
//        });
...
//        });
        ...
        registerOnlineOnlyTag(ItemTag.class, "item_on_cursor", (attribute, object) -> {
            ServerPlayer serverPlayer = object.getPlayerEntity();
            return new ItemTag(object.getPlayerEntity().containerMenu.getCarried()); //containermenu is the opened container if one is opened or else the players inventory
        });
        ...
        registerOnlineOnlyTag(ElementTag.class, "held_item_slot", (attribute, object) -> {
            return new ElementTag(object.getPlayerEntity().getInventory().selected + 1);
        });
...
//            return new ElementTag(object.getPlayerEntity().getUUID() + "|" + skin.substring(0, semicolon) + "|" + object.getName());
//        });
        ...
        registerOnlineOnlyTag(ObjectTag.class, "attack_cooldown", (attribute, object) -> {
            ...
            if (attribute.startsWith("duration", 2)) {
                ...
                attribute.fulfill(1);
                return new ElementTag(object.getPlayerEntity().getAttackStrengthScale(0) * 100);
            }
...
//        });
        ...
        registerOnlineOnlyTag(ElementTag.class, "ip_address", (attribute, object) -> {
            return new ElementTag(object.getPlayerEntity().connection.connection.getRemoteAddress().toString());
        });
        ...
        registerOnlineOnlyTag(ElementTag.class, "ip", (attribute, object) -> {
            ...
            //todo
//            String host = object.getPlayerEntity().connection.connection.getRemoteAddress().getHostName();
...
//        });
        ...
        registerOnlineOnlyTag(LocationTag.class, "compass_target", (attribute, object) -> {
            BlockPos target = object.getPlayerEntity().getRespawnPosition();
            if (target != null) {
...
//        });
        ...
        registerOfflineTag(ElementTag.class, "can_fly", (attribute, object) -> {
            if (object.isOnline()) {
                return new ElementTag(object.getPlayerEntity().getAbilities().mayfly);
            }
            else {
        ...
        registerOfflineTag(ElementTag.class, "fly_speed", (attribute, object) -> {
            if (object.isOnline()) {
                return new ElementTag(object.getPlayerEntity().getAbilities().getFlyingSpeed());
            }
            else {
        ...
        registerOfflineTag(ElementTag.class, "walk_speed", (attribute, object) -> {
            if (object.isOnline()) {
                return new ElementTag(object.getPlayerEntity().getAbilities().getWalkingSpeed());
            }
            else {
        ...
        registerOfflineTag(ElementTag.class, "saturation", (attribute, object) -> {
            if (object.isOnline()) {
                return new ElementTag(object.getPlayerEntity().getFoodData().getSaturationLevel());
            }
            else {
        ...
        registerOfflineTag(ElementTag.class, "gamemode", (attribute, object) -> {
            if (object.isOnline()) {
                return new ElementTag(object.getPlayerEntity().gameMode.getGameModeForPlayer().getName());
            }
        ...
        registerOnlineOnlyTag(ElementTag.class, "ping", (attribute, object) -> {
            return new ElementTag(object.getPlayerEntity().latency);
        });
        ...
        registerOnlineOnlyTag(ElementTag.class, "is_flying", (attribute, object) -> {
            return new ElementTag(object.getPlayerEntity().isFallFlying());
        });
        ...
        if (!Denizen.supportsPaper || NMSHandler.getVersion().isAtMost(NMSVersion.v1_18)) {
            registerOnlineOnlyTag(ElementTag.class, "is_sneaking", (attribute, object) -> {
                return new ElementTag(object.getPlayerEntity().isCrouching());
            });
        }
        ...
        registerOnlineOnlyTag(ElementTag.class, "has_advancement", (attribute, object) -> {
            ...
            AdvancementProgress progress = object.getPlayerEntity().getAdvancements().getOrStartProgress(adv);
        ...
        registerOnlineOnlyTag(ListTag.class, "advancements", (attribute, object) -> {
            ListTag list = new ListTag();
            Collection<Advancement> advancementList = ServerLifecycleHooks.getCurrentServer().getAdvancements().getAllAdvancements();
            for (Advancement advancement : advancementList)
            {
                if (object.getPlayerEntity().getAdvancements().getOrStartProgress(advancement).isDone()) {
                    list.add(advancement.getId().toString());
                }
            }
            return list;
        }, "list_advancements");
        ...
        registerOnlineOnlyTag(DurationTag.class, "time_asleep", (attribute, object) -> {
            return new DurationTag((long) object.getPlayerEntity().getSleepTimer());
        });
...
//            if (object.getPlayerEntity(). != null) {
...
//        });
        ...
        registerOnlineOnlyTag(ObjectTag.class, "spectator_target", (attribute, object) -> {
            ...
            Entity target = serverPlayer.getCamera(); //returns this if null
...
//        });
        ...
        registerOnlineOnlyTag(EntityTag.class, "fish_hook", (attribute, object) -> {
            FishingHook hook = object.getPlayerEntity().fishing;
            if (hook == null) {
                return null;
            }
...
//        });
        ...
        tagProcessor.registerTag(ElementTag.class, "scoreboard_team_name", (attribute, object) -> {
            ServerPlayer serverPlayer = object.getPlayerEntity();
            PlayerTeam playerTeam = serverPlayer.getScoreboard().getPlayerTeam(serverPlayer.getScoreboardName());
            if (playerTeam == null) {
                return null;
            }
            return new ElementTag(playerTeam.getName());
        });
...
//        }
        ...
        if (mechanism.matches("item_slot") && mechanism.requireInteger()) {
            if (isOnline()) {
                getPlayerEntity().getInventory().selected = mechanism.getValue().asInt() - 1;
            }
            else {
...
//        }
        ...
        if (mechanism.matches("item_on_cursor") && mechanism.requireObject(ItemTag.class)) {
            getPlayerEntity().containerMenu.setCarried(mechanism.valueAsType(ItemTag.class).getItemStack());
        }
        ...
        if (mechanism.matches("award_advancement")) {
            ...
            AdvancementProgress prog = getPlayerEntity().getAdvancements().getOrStartProgress(adv);
        ...
        if (mechanism.matches("revoke_advancement")) {
            ...
            AdvancementProgress prog = getPlayerEntity().getAdvancements().getOrStartProgress(adv);
            for (String criteria : prog.getCompletedCriteria()) {
                prog.revokeProgress(criteria);
            }
        }
...
//        }
        ...
        if (mechanism.matches("health") && mechanism.requireDouble()) {
            setHealth(mechanism.getValue().asFloat());
        }
        ...
        if (mechanism.matches("saturation") && mechanism.requireFloat()) {
            if (isOnline()) {
                getPlayerEntity().getFoodData().setSaturation(mechanism.getValue().asFloat());
            }
            else {
...
//        }
        ...
        if (mechanism.matches("fly_speed") && mechanism.requireFloat()) {
            ...
            if (isOnline()) {
                getPlayerEntity().getAbilities().setFlyingSpeed(val);
            }
            else {
...
//        }
        ...
        // <PlayerTag.gamemode>
        // -->
        if (mechanism.matches("gamemode") && mechanism.requireEnum(GameType.class)) {
            setGameMode(GameType.valueOf(mechanism.getValue().asString().toUpperCase()));
        }
//todo
...
//        }
        ...
        // -->
        //todo probably requires sending packets
//        if (mechanism.matches("display_name")) {
//            getPlayerEntity().getDisplayName() setDisplayName(mechanism.getValue().asString());
//            return;
//        }
...
//        }
        ...
        if (mechanism.matches("exhaustion") && mechanism.requireFloat()) {
            if (isOnline()) {
                getPlayerEntity().getFoodData().setExhaustion(mechanism.getValue().asFloat());
            }
            else {
...
//            if (isOnline()) {
//                for (Entity ent : getPlayerEntity().getLevel().getEntities().getAll()) {
...
//        }
        ...
        if (mechanism.matches("fake_mount_health")) {
            if (!isOnline() || (getPlayerEntity().getVehicle() == null)) {
...
//        }
        ...
        if (mechanism.matches("spectator_target")) {
            ...
            if (mechanism.hasValue()) {
                ...
                serverPlayer.setCamera(mechanism.valueAsType(EntityTag.class).getBukkitEntity());
            }
            else if (getPlayerEntity().gameMode.getGameModeForPlayer() == GameType.SPECTATOR) {
        ...
        if (mechanism.matches("open_book")) {
            ItemStack book = getPlayerEntity().getMainHandItem();
            if (book.getItem() == Items.WRITTEN_BOOK) {
                getPlayerEntity().openItemGui(book, InteractionHand.MAIN_HAND);
            }
        }
        ...
        if (mechanism.matches("open_offhand_book")) {
            ItemStack book = getPlayerEntity().getOffhandItem();
            if (book.getItem() == Items.WRITTEN_BOOK) {
                getPlayerEntity().openItemGui(book, InteractionHand.OFF_HAND);
            }
        }
        ...
        if (mechanism.matches("show_book")
            ...
            getPlayerEntity().openItemGui(book.getItemStack(), InteractionHand.MAIN_HAND);
        }
...
//            }
//            getPlayerEntity().getRecipeBook().add(keys);
//        }
        ...
        // -->
        //todo change utility to get recipe
...
//                }
//                if (!NMSHandler.getVersion().isAtLeast(NMSVersion.v1_18)) {
//                    NMSHandler.packetHelper.showSignEditor(getPlayerEntity().level.getBlockState(0,0,0), state.getLocation());
//                    return;
//                }
...
//        }
        ...
        if (mechanism.matches("is_op") && mechanism.requireBoolean()) {
            ...
            ServerLifecycleHooks.getCurrentServer().getPlayerList().op(getOfflinePlayer().gameProfile);
        }
