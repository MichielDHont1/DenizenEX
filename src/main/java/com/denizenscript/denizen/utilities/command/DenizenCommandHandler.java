...
public class DenizenCommandHandler {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("denizen").requires((SourceStack) -> {
            return SourceStack.hasPermission(3);
        }).then(Commands.literal("submit").executes((SourceStack) -> {
            return submit(SourceStack.getSource());
        })).then(Commands.literal("debug").executes((SourceStack) -> {
...
//            min = 1, max = 3, permission = "denizen.submit")
    public static int submit(CommandSourceStack command){
        Player sender;
...
//            min = 1, max = 5, permission = "denizen.debug", flags = "scbroevnipfl")
    public static int debug(CommandSourceStack command, String args){
        Player sender;
        String[] flags;
        if(command.getEntity() instanceof Player){
            sender = (Player) command.getEntity();
        ...
        for (String flag : flags) {
            ...
            if (flag.contains("-v") || flag.contains("-verbose")) {
...
//            min = 1, max = 3, permission = "denizen.basic")
    public static int version(CommandSourceStack command, String args){
        Player sender;
        String[] flags;
        if(command.getEntity() instanceof Player){
            sender = (Player) command.getEntity();
...
//            min = 1, max = 3, permission = "denizen.basic")
        public static int save(CommandSourceStack command){
            Player sender;
            String[] flags;
            if(command.getEntity() instanceof Player){
                sender = (Player) command.getEntity();
...
//            min = 1, max = 3, permission = "denizen.basic", flags = "a")
    public static int reload(CommandSourceStack command, @Nullable String arguments){
        Player sender;
        String[] flags;
        if(command.getEntity() instanceof Player){
            sender = (Player) command.getEntity();
        ...
        Messaging.send(sender, "");
        Messaging.send(sender, "<f>Specify which parts to reload. Valid options are: SAVES, NOTES, CONFIG, SCRIPTS, ALL");
...
//            min = 1, max = 4, permission = "denizen.basic")
    public static int scripts(CommandSourceStack command, @Nullable String arguments) {
        ...
        if (PageNumber != -1 && !paginator.sendPage(sender, PageNumber)) {
            String message ="The page " + Integer.toString(PageNumber) + " does not exist!";
            return -1;
        }
        return com.mojang.brigadier.Command.SINGLE_SUCCESS;
    }
