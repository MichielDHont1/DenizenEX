...
//                DataResult<ResourceLocation> result = ResourceLocation.read(entry);
//
//                    if (MaterialTag.advancedMatchesInternal(new Item(), entry, true)) {
...
//        }
    ...
    public static void registerShapedRecipe(ItemScriptContainer container, ItemStack item, List<String> recipeList, String internalId, String group) {
        ...
        //RecipeManager.fromJson(resource, jsonobj, context);
//todo recipe builder
            ...
            try {
                if (container.contains("recipes", Map.class)) {
                    ...
                    for (StringHolder key : section.getKeys(false)) {
                        ...
                        ItemStack item = container.getCleanReference().getItemStack();
                        if (subSection.contains("output_quantity")) {
                            item.setCount(Integer.parseInt(getString.apply("output_quantity")));
                        }
                        switch (type) {
...
//                            case "brewing" -> PaperAPITools.instance.registerBrewingRecipe(internalId, item, getString.apply("input"), getString.apply("ingredient"), container);
                ...
                if (container.contains("RECIPE", List.class)) {
                    ...
                    registerShapedRecipe(container, container.getCleanReference().getItemStack(), container.getStringList("RECIPE"), getIdFor(container, "old_recipe", 0), "custom");
                }
                if (container.contains("SHAPELESS_RECIPE", String.class)) {
                    BukkitImplDeprecations.oldRecipeScript.warn(container);
                    registerShapelessRecipe(container, container.getCleanReference().getItemStack(), container.getString("SHAPELESS_RECIPE"), getIdFor(container, "old_shapeless", 0), "custom", null);
                }
                if (container.contains("FURNACE_RECIPE", String.class)) {
                    BukkitImplDeprecations.oldRecipeScript.warn(container);
                    registerFurnaceRecipe(container, container.getCleanReference().getItemStack(), container.getString("FURNACE_RECIPE"), 0, 40, "furnace", getIdFor(container, "old_furnace", 0), "custom", null);
                }
            }
            catch (Exception ex) {
...
//    }
    ...
    private static ItemStack AIR = new ItemStack(Items.AIR);
