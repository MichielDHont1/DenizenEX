...
public class NarrateCommand extends AbstractCommand {
    ...
    public void execute(ScriptEntry scriptEntry) {
        ...
        if (targets == null) {
            Debug.log(FormattedTextHelper.parse(format != null ? format.getFormattedText(text, scriptEntry) : text, ChatColor.WHITE).getText());
            return;
        }
        for (PlayerTag player : targets) {
            if (player != null) {
                ...
                TextComponent component = FormattedTextHelper.parse(format != null ? format.getFormattedText(personalText, scriptEntry) : personalText, ChatColor.WHITE);
                if (fromId == null) {
                    player.getPlayerEntity().sendMessage(component, Util.NIL_UUID);
                }
                else {
                    player.getPlayerEntity().sendMessage(component, fromId);
                }
            }
            else {
